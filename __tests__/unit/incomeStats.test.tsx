...
(axios.get as jest.Mock).mockImplementation((url: string, { params }) => {
  if (url === "/api/fetchUser" && params.email === mockUser.email) {
    return Promise.resolve({ data: { id: "123" } });
  } else if (url === "/api/fetchIncomes" && params.userId === "123") {
